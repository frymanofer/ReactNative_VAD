...
function App(): React.JSX.Element {
  ...
  const [timeSinceVoice, setTimeSinceVoice] = useState(0);
...
//  const { stopListening, startListening, loadModel, setKeywordDetectionLicense} = useModel();
   ...
   /* const keywordCallback = async (keywordIndex: any) => {
    ...
    const updateVoiceProps = async () => {
      try {
        ...
        setTimeSinceVoice(voiceProps.lastTimeHumanVoiceHeard);
      } catch (error) {
  ...
  */
        const interval = setInterval(updateVoiceProps, 500);
  ...
  <LinearGradient
    ...
    <ScrollView
      ...
      { backgroundColor: 
        ...
        <Text style={styles.title}>Time Since Voice: {timeSinceVoice} ms</Text>
      </View>
    </ScrollView>
    </LinearGradient>
);
